...
from torch_geometric.nn.inits import reset as pyg_reset
...
class GINE_Player(nn.Module):
    """Graph Isomorphism Network with Edge features (GINE) layer.
    """
    def __init__(self, dim_in, dim_out, dropout, residual, ffn, act_fn_ff=None):
        super().__init__()
        self.dim_in = dim_in
        ...
        self.raw_layer = pyg_nn.GINEConv(gin_nn)
        ...
        if self.ffn:
            ...
            self.act_fn_ff = activation_resolver(act_fn_ff)()
            if self.batch_norm:
    ...
    def forward(self, batch):
        ...
        batch.x = self.raw_layer(batch.x, batch.edge_index, batch.edge_attr)

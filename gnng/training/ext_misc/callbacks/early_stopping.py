...
Early Stopping
^^^^^^^^^^^^^^
This file is substantially modified from `lightning.pytorch.callbacks.EarlyStopping` to fit my own fabric-based trainer
...
from lightning.pytorch.callbacks import EarlyStopping as ee
...
class EarlyStopping(Callback):
    ...
    def __init__(
        ...
        check_on_train_epoch_end: Optional[bool] = False,  # False to enable check per valid epoch
    ...
    def _validate_condition_metric(self, logs: Dict[str, Tensor], trainer: Any) -> bool:
        ...
        if monitor_val is None:
            if self.strict and trainer.state == "validating":
    ...
    def _should_skip_check(self, trainer: Any) -> bool:
        return trainer.state not in ("training", "validating") or trainer.sanity_checking
    ...
    def _run_early_stopping_check(self, trainer: Any) -> None:
        ...
        logs = trainer.current_fit_epoch_summary
        ...
        should_stop = trainer.fabric.strategy.reduce_boolean_decision(should_stop, all=False)
    ...
    @staticmethod
    def _log_info(trainer: Any, message: str, log_rank_zero_only: bool) -> None:
        rank = trainer.fabric.global_rank if trainer.fabric.world_size > 1 else None

...
from typing_extensions import override, Sequence
...
class WandbLogger(Logger):
    ...
    **Log gradients, parameters and model topology**
    ...
    .. code-block:: python
        ...
        # change log frequency of gradients and parameters (100 losses by default)
    ...
    **Downloading and Using Artifacts**
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        project = project or os.environ.get("WANDB_PROJECT", "fabric_logs")
        ...
        # to create a run in the main process at initialization such that `self.version` won't return `Version_None`
        self._experiment = self.experiment

    def _need_init_new_run(self, run):
        if self._wandb_init.get("mode") == "disabled":
            return False
        need_new_run = True
        if run is not None:
            existing_run_info = {
            ...
            input_wb_init_kwargs = {k: self._wandb_init.get(k, "") for k in existing_run_info}
            if input_wb_init_kwargs["tags"] == "":
                input_wb_init_kwargs["tags"] = ()

            if existing_run_info == input_wb_init_kwargs:  # we need a existing run with same init kwargs
                need_new_run = False
        return need_new_run
    ...
    def experiment(self) -> Union["Run", "RunDisabled"]:
        ...
        if self._experiment is None:
            ...
            if wandb.run is not None:
                if self._need_init_new_run(wandb.run):
                    self._experiment = wandb.init(**self._wandb_init)
                else:
                    # wandb process already created in this instance
                    rank_zero_warn(
                        "There is a wandb run already in progress and newly created instances of `WandbLogger` will "
                        "reuse this run. If this is not desired, call `wandb.finish()` before create `WandbLogger`."
                    )
                    self._experiment = wandb.run
            elif attach_id is not None and hasattr(wandb, "_attach"):
                # attach to wandb process referenced
                self._experiment = wandb._attach(attach_id)
            else:  # no run instance
                # create new wandb process
                self._experiment = wandb.init(**self._wandb_init)

                # define default x-axis (name)
    ...
    @rank_zero_only
    def log_metrics(
        self, metrics: Mapping[str, float], step: Optional[int] = None, epoch: Optional[int] = None
    ) -> None:
        assert rank_zero_only.rank == 0, "experiment tried to log from global_rank != 0"
        metrics = _add_prefix(metrics, self._prefix, self.LOGGER_JOIN_CHAR)
        if step is not None and epoch is not None:
            raise RuntimeError(f"One log action can only be either `step-level` or `epoch-level`.")
        elif epoch is not None:
            self.experiment.log(dict(metrics, **{"epoch": epoch}))
    ...
    def version(self) -> Optional[str]:
        ...
        return self._experiment.id if self._experiment is not None else self._id
    ...
    def log_confusion_matrix(
        ...
        title = title or key
        cm_table = cm2wtp_confusion_matrix(cm, class_names, title, split_table)
        self.log_metrics({key: cm_table}, step=step)


def cm2wtp_confusion_matrix(
    cm: Union[np.array, Tensor],
    class_names: Optional[Sequence[str]] = None,
    title: Optional[str] = None,
    split_table: Optional[bool] = False,
    ...
    if class_names is not None:
        ...
        assert n_classes <= num_class_names, "Higher predicted index than number of classes"
        assert n_classes <= num_class_names, "Higher label class index than number of classes"
    else:
